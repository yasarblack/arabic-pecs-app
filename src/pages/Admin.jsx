...
export default function Admin() {
  ...
  async function loadCards(currentSession = session) {
  ...
  async function handleDelete(cardId) {
    ...
    const result = await response.json().catch(() => ({}))
    if (!response.ok) {
      setError(result.error || 'فشل حذف البطاقة')
      return
    }
  ...
  if (loading) {
    return <div className="admin-page"><div className="admin-loading">جاري التحميل…</div></div>
  }
    ...
    return (
      <div className="admin-page">
        <div className="admin-shell admin-login-card">
          <button className="admin-back" onClick={() => navigate('/')}>← العودة</button>
          ...
          <form onSubmit={handleLogin} className="admin-form">
            ...
            <button className="admin-primary" type="submit">دخول</button>
          </form>
        </div>
      </div>
    )
      ...
      <div className="admin-shell">
        <header className="admin-topbar">
          ...
          <div className="admin-actions">
            <button className="admin-secondary" onClick={() => navigate('/profiles')}>لوحة الأطفال</button>
            <button className="admin-secondary" onClick={handleLogout}>خروج</button>
          </div>
        </header>
        ...
        <section className="admin-panel">
          ...
          <form onSubmit={handleAdd} className="admin-form">
                ...
                <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} required>
                  {categories.map((category) => (
                    <option key={category.id} value={category.id}>
                      {category.icon ? `${category.icon} ` : ''}{category.name_ar}
                    </option>
                  ))}
                </select>
              </label>
            ...
            <div className="admin-row">
              ...
              <label>الصوت العربي <span>(اختياري)</span>
                <input
                  type="file"
                  accept="audio/*"
        ...
        <section className="admin-panel">
          <div className="admin-section-title">
            ...
            <button className="admin-secondary" onClick={() => loadCards(session)}>تحديث</button>
          </div>
              ...
              {cards.map((card) => (
                <article className="admin-card" key={card.id}>
                  ...
                  <div className="admin-card-body">
                    <h3>{card.label_ar}</h3>
                    <p>{card.category_name || 'بدون فئة'} · صعوبة {card.difficulty_level ?? 1}</p>
                    {card.audio_url && <audio controls src={card.audio_url} />}
                    <button className="admin-danger" onClick={() => handleDelete(card.id)}>حذف البطاقة</button>
                  </div>
                </article>
              ))}
            </div>
